...
You are all divided into 4 groups. The Rmarkdown chunks specific for each group are marked. Feel free to work through all chunks for the other groups too. None of the group-specific Rmarkdown chunks need to be run to get the main results of the script.

## Load libraries, set user directory paths, and read data into a Seurat object. 
```{r}
library(Seurat) 
library(dplyr)
library(ggplot2)
...
## Group 1: Interpreting the principle components from PCA analysis
What biological information is found in the principle components, and how many principal components should we use to describe our transcriptomic data? The principle components can indicate which genes are important in different cell subsets. 
```{r PCA_analysis}
...
# After how many PCs do you start to see lack of separation in the cells?

# Elbow plot. You can read more about PCA plots here
...
## Clustering to identify cell subsets
...
# Visualize cells, colored with amount of detected SARS-CoV-2 RNA.
gcdata$SARSCoV2_Total_Corrected <- as.integer(gcdata$SARSCoV2_Total_Corrected)
...
We will see how changing the parameters used in the clustering and by visualization algorithms affect our results. 
```{r visualization}
...
# Try a range of perplexity parameters from 1-100. Also check out https://pair-code.github.io/understanding-umap/ for further explanation.
...
## Identify the genes differentially expressed by each cell subset
By looking at genes that are differentially expressed in one cell subset relative to all other cell subsets, we can identify the cell type or cell state corresponding to that cell subset. Identifying the cell types and states tells us about the composition of the tissue, and allows us to ask focused questions on what may be changing within a specific cell type.
```{r DE_genes}
# Select which differential expression (DE) test to use in Seurat. We will use the wilcox test.
# Look at the documentation of FindAllMarkers (?FindAllMarkers) to better understand 
...
# Visualize the expression of the DE genes for each cell subset. 
...
# We will only measure DE genes for those annotated cell subsets containing more than 100 cells.
# To do so, we can subset the original Seurat object (gcdata) to a new Seurat object gcdata.sub.
Idents(gcdata) <- "Coarse_Cell_Annotations"
...
# Visualize the expression of the DE genes for each annotated cell subset. 
top10genes <- gcdata.markers %>% filter(avg_log2FC > 0) %>% group_by(cluster) %>% top_n(10, avg_log2FC)
genes <- top10genes$gene
...
# This is a bit difficult to look at. Let us try another visualization.
...
# Within ciliated cells, compare gene expression from COVID-19 patients to gene expression from control patients.
Idents(gcdata) <- "Coarse_Cell_Annotations"
...
# Visualize the expression of the DE genes. 
...
# Try repeating the above analysis for different cell subsets and saving your results as gene expression heat maps.
Idents(gcdata) <- "Coarse_Cell_Annotations"
